...
from PyQt5.Qt import (QDialog, QVBoxLayout, QHBoxLayout, QLabel, QLineEdit,
                      ...
                      QSplitter, QGroupBox, QComboBox, QSlider, QSpinBox,
                      Qt, QSize, QFont, QProgressBar, QTimer, pyqtSignal,
                      QWidget, QGridLayout, QCheckBox, QToolButton, QMenu,
                      QAction, QMessageBox)
import asyncio
...
class ResultCard(QWidget):
    ...
    def _setup_ui(self):
        ...
        # Book info
        book_info = QLabel(f"<b>{self.result.book_title}</b><br>"
        ...
        similar_btn.clicked.connect(lambda: self.similar_clicked.emit(self.result.chunk_id))
...
class SemanticSearchDialog(QDialog):
    ...
    def _load_settings(self):
        ...
        threshold = int(self.config.get('search_options.similarity_threshold', 0.7) * 100)
    ...
    def perform_search(self):
        ...
        try:
            ...
            future = asyncio.run_coroutine_threadsafe(
                self.search_engine.search(query, options),
                self.loop
            )
