...
class SemanticSearchDB:
    ...
    def _create_connection(self) -> sqlite3.Connection:
        ...
        try:
            ...
            if not loaded:
                logger.warning("sqlite-vec extension not found. Vector search will be limited.")
    ...
    def _create_schema(self, conn: sqlite3.Connection):
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_books_indexed ON books(last_indexed)")
    ...
    def store_embedding(self, book_id: int, chunk: 'Chunk', embedding: np.ndarray) -> int:
        """
        ...
        with self.transaction() as conn:
            ...
            cursor = conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (book_id, chunk.index, chunk.text, 
    ...
    def search_similar(self, embedding: np.ndarray, limit: int = 20,
        ...
        try:
            ...
            results = self._conn.execute(
                query, 
                [embedding.astype(np.float32).tobytes()] + params + [limit]
            ).fetchall()
        ...
        for row in results:
            ...
            similarity = 1.0 - (row['distance'] / 2.0) if 'distance' in row else row.get('similarity', 0)
    ...
    def _search_similar_fallback(self, embedding: np.ndarray, limit: int,
    ...
    def clear_book_embeddings(self, book_id: int):
        ...
        with self.transaction() as conn:
            ...
            if chunk_ids:
                ...
                try:
                    conn.execute(f"DELETE FROM vec_embeddings WHERE chunk_id IN ({placeholders})", 
                               chunk_ids)
                except sqlite3.OperationalError:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        # Database size
        stats['database_size'] = os.path.getsize(self.db_path) if self.db_path.exists() else 0
